"""
Training All 5 ML and DL Models Specifically on Offensive-24K-T2 (Target Identification) Dataset
Models:
  - ML: Linear SVM (Calibrated), Multinomial Naive Bayes, Random Forest (with TF-IDF Vectorizer)
  - DL: 1D CNN, Bidirectional LSTM (with Tokenizer and Sequence Embeddings)
"""
...
DATASET_FILE = os.path.join(BASE_PATH, "Offensive-24K-T2(Target Identification).xlsx")

def clean_text(text):
    """Clean and normalize Urdu/Roman Urdu text while preserving word context"""
    if pd.isna(text):
        return ""
    text = str(text)
    # Remove URLs
...
print("="*70)
print(" STEP 1: Loading & Preprocessing Offensive-24K-T2 Dataset")
print("="*70)
...
def parse_tag(x):
    if isinstance(x, str):
        x = x.strip().upper()
        if x in ['TIN', '1', 'OFF', 'TARGETED']:
            return 1
        elif x in ['UNT', '0', 'NOT', 'UNTARGETED']:
            return 0
    try:
        val = int(x)
...
print(f"  - Class 0 (Untargeted/General): {sum(df['label'] == 0)} ({sum(df['label'] == 0)/len(df)*100:.1f}%)")
print(f"  - Class 1 (Targeted Insult)  : {sum(df['label'] == 1)} ({sum(df['label'] == 1)/len(df)*100:.1f}%)")
...
with open(os.path.join(MODEL_DIR, "svm.pkl"), "wb") as f:
    pickle.dump(svm, f)
print("✓ Saved models/svm.pkl (with calibrated probabilities)")
...
# ─────────────────────────────────────────────
print("\n" + "="*75)
print("  FINAL EVALUATION RESULTS — ALL 5 MODELS ON OFFENSIVE-24K-T2")
print("="*75)
...
print("="*75)
print("All 5 Models Trained & Successfully Saved to models/ directory for T2!")
print("="*75)
